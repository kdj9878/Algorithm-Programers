...
public class NewsClustering {
    ...
    합집합 A ∪ B는 원소 "1"을 max(3, 5)인 5개 가지게 된다. 다중집합 A = {1, 1, 2, 2, 3},
    다중집합 B = {1, 2, 2, 4, 5}라고 하면, 교집합 A ∩ B = {1, 2, 2}, 합집합 A ∪ B = {1, 1, 2, 2, 3, 4, 5}가 되므로,
    ...
    void createAggregate(ArrayList<String> list, String s){
        char[] chaArr = s.toCharArray();
        int count = 0;
        String temp = "";
        for(int i = 0; i < chaArr.length; i++){
            temp += String.valueOf(chaArr[i]);
            count++;
            // 두 번 뽑았을 경우
            if(count%2 == 0){
    ...
    int countAggregate(String flag, Map<String, Integer> firstMap, Map<String, Integer> secondMap){
        int returnInt = 0;
        Map<String, Integer> countMap = new HashMap<>();
        Set<String> firstSet;
        Set<String> secondSet;
        firstSet = firstMap.keySet();
        secondSet = secondMap.keySet();

        if(flag.equals("min")){
            for(String key : firstSet){
                if(secondSet.contains(key)){
                    countMap.put(key, Math.min(firstMap.get(key), secondMap.get(key)));
                }
            }

            for(String key : secondSet){
                if(firstSet.contains(key)){
                    countMap.put(key, Math.min(secondMap.get(key), firstMap.get(key)));
                }
            }

            for(String key : countMap.keySet()){
                returnInt += countMap.get(key);
            }
        }
        else{
            for(String key : firstSet){
                if(secondSet.contains(key)){
                    countMap.put(key, Math.max(firstMap.get(key), secondMap.get(key)));
                }
                else{
                    countMap.put(key, firstMap.get(key));
                }
            }

            for(String key : secondSet){
                if(firstSet.contains(key)){
                    countMap.put(key, Math.max(secondMap.get(key), firstMap.get(key)));
                }
                else{
                    countMap.put(key, secondMap.get(key));
                }
            }
    ...
    void newsClustering(){
        ...
        createAggregate(list1, str1.toLowerCase());
        createAggregate(list2, str2.toLowerCase());

        // 각 요소의 개수를 구함
        for(String s : list1) firstMap.put(s, firstMap.getOrDefault(s, 0) + 1);
        for(String s : list2) secondMap.put(s, secondMap.getOrDefault(s, 0) + 1);

        int intersection = countAggregate("min", firstMap, secondMap);
        int union = countAggregate("max", firstMap, secondMap);
        ...
        // int similarity = (intersection / union)
