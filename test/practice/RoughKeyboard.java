...
public class RoughKeyboard {
    ...
    void roughKeyboard(){
        ...
        for(int z = 0; z < targets.length; z++){
            for(int i = 0; i < targets[z].length(); i++){
                String temp = targets[z].substring(i, i+1);
                int indexMin = 101;
                //각 keymap 원소에서 indexOf를 하여 최소 횟수를 구한다.
                for(int j = 0; j < keymap.length; j++){
                    if(keymap[j].indexOf(temp) > -1 && keymap[j].indexOf(temp) < indexMin){
                        indexMin = keymap[j].indexOf(temp); //몇 번 눌렀는지
                    }
                }
                if(indexMin == 101){
                    answer[z] = -1;
